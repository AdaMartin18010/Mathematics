...
- [3. 一阶逻辑：量词、谓词与模型](#3-一阶逻辑量词谓词与模型)
  ...
  - [国际化对齐补全：一阶逻辑的语法与语义（对齐 Wikipedia/EoM）](#国际化对齐补全一阶逻辑的语法与语义对齐-wikipediaeom)
    - [一、语法（Syntax）](#一语法syntax)
...
## 国际化对齐补全：一阶逻辑的语法与语义（对齐 Wikipedia/EoM）

### 一、语法（Syntax）

- 语言L的符号：常元、函数符、谓词符、联结词、量词(∀,∃)、变量
- 项（term）：常元或函数符作用于若干项
- 公式（formula）：原子公式P(t₁,...,tₙ) 在联结词与量词下封闭；自由/约束变量
- 前束范式（Prenex normal form）：任意公式等价于前面量词前缀+量词自由的矩阵

### 二、语义（Semantics）

- 结构/模型 M=(|M|; f^M, P^M, c^M) 给出符号的解释
- 赋值 s 映射变量到 |M|
- 解释与满足：写作 M,s ⊨ φ；句子φ无自由变元时写 M ⊨ φ
- 理论T的模型：M ⊨ T；语义蕴涵：T ⊨ φ；有效性：⊨ φ

### 三、核心定理（陈述）

- 健全性（Soundness）：T ⊢ φ ⇒ T ⊨ φ
- 完备性（Gödel Completeness）：T ⊨ φ ⇒ T ⊢ φ（可给出亨金扩张证明纲要）
- 紧致性（Compactness）：若T的每个有限子集可满足，则T可满足（由完备性或乌尔森引理）
- 洛文海姆–斯科伦定理：
  - 下LS：若T有无限模型，则T有可数模型（在可数语言中）
  - 上LS：若T有无限模型，则对任意κ≥|L|存在基数为κ的模型
- 斯科伦化（Skolemization）：引入函数符消去存在量词，保持可满足性

### 四、例子

- 群论语言L={·,^{-1},e}；PA的一阶语言L={0,S,+,×,<}
- 实闭域理论RCF的量词消去性质（陈述）

### 五、参考

- Enderton, A Mathematical Introduction to Logic；Marker, Model Theory
- [Wikipedia: First-order logic](https://en.wikipedia.org/wiki/First-order_logic)
- [EoM: Completeness theorem](https://encyclopediaofmath.org/wiki/Completeness_theorem)
