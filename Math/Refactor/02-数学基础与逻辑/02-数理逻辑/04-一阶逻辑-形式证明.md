...
- [4. 一阶逻辑：形式证明](#4-一阶逻辑形式证明)
  ...
  - [国际化对齐补全：形式证明与完备性纲要（对齐 Wikipedia/EoM）](#国际化对齐补全形式证明与完备性纲要对齐-wikipediaeom)
    - [一、证明系统概览](#一证明系统概览)
    - [二、完备性（亨金构造）证明思路](#二完备性亨金构造证明思路)
    - [三、紧致性证明思路](#三紧致性证明思路)
    - [四、参考](#四参考)
...
## 国际化对齐补全：形式证明与完备性纲要（对齐 Wikipedia/EoM）

### 一、证明系统概览

- 希尔伯特体系：少量公理模式 + Modus Ponens
- 自然演绎：假设-释放与引入/消去规则（→, ∧, ∨, ¬, ∀, ∃）
- 序列演算（Gentzen）：割消去定理（陈述）

### 二、完备性（亨金构造）证明思路

1) 预备：将公式前束化，Skolem化保可满足性
2) 语言扩张：为每个存在公式∃x φ(x)引入亨金常元c_φ
3) 一致性保持：采用Lindenbaum极大一致集Γ
4) 构造模型：以项域Term/≈为载体，解释符号；证明Term-模型满足Γ
5) 得出：若T⊨φ则T⊢φ

### 三、紧致性证明思路

- 由完备性：若T不可满足，则存在有限不可满足子集
- 由乌尔森引理（超滤/极大理想）给出另一证明路线（陈述）

### 四、参考

- Enderton, Mendelson；Hinman, Fundamentals of Mathematical Logic
- [Wikipedia: Gödel's completeness theorem](https://en.wikipedia.org/wiki/G%C3%B6del%27s_completeness_theorem)
- [EoM: Compactness theorem](https://encyclopediaofmath.org/wiki/Compactness_theorem)
