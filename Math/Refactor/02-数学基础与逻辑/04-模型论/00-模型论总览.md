...
- [目录](#目录)
  ...
  - [1.1.1. 知识点国际标准定义补全](#111-知识点国际标准定义补全)
    ...
    - [结构/模型（Structure/Model）](#结构模型structuremodel)
    - [同构（Isomorphism）](#同构isomorphism)
    - [可定义性（Definability）](#可定义性definability)
    - [理论（Theory）](#理论theory)
    - [范畴性（Categoricity）](#范畴性categoricity)
    - [完备性（Completeness, Model Theory）](#完备性completeness-model-theory)
  - [哲学批判与反思](#哲学批判与反思)
  - [国际化对齐补全：模型论基础（对齐 Wikipedia/EoM）](#国际化对齐补全模型论基础对齐-wikipediaeom)
    - [一、模型与理论](#一模型与理论)
    - [二、紧致性与Löwenheim–Skolem](#二紧致性与löwenheimskolem)
    - [三、量词消去与可定义性](#三量词消去与可定义性)
    - [四、稳定性与分类理论（概览）](#四稳定性与分类理论概览)
    - [五、参考](#五参考)
...
- [99-一页学习路线与速查表](./99-一页学习路线与速查表.md)
...
- **英文名**：Model Theory
- **国际标准表述**：The branch of mathematical logic studying the relationships between formal languages and their interpretations, or models.
- **符号**：无专用符号
- **简明解释**：研究形式语言与其模型之间关系的逻辑分支。
- **典型应用**：逻辑基础、代数结构分析。
- **相关定理引用**：紧致性定理、范畴性。

### 结构/模型（Structure/Model）

- **英文名**：Structure / Model
- **国际标准表述**：A set together with interpretations for the symbols of a formal language, providing meaning to the language.
- **符号**：$\mathcal{M} = (D, \mathcal{I})$
- **简明解释**：为形式语言赋予具体含义的数学对象。
- **典型应用**：模型论、代数、分析。
- **相关定理引用**：同构、范畴性。
...
- **英文名**：Isomorphism
- **国际标准表述**：A bijective mapping between two structures that preserves all relations and operations.
- **符号**：$f: \mathcal{M} \cong \mathcal{N}$
- **简明解释**：结构间一一对应且保持结构性质的映射。
- **典型应用**：结构分类、范畴性。
- **相关定理引用**：范畴性、可定义性。
...
- **英文名**：Definability
- **国际标准表述**：A subset or relation is definable in a structure if there exists a formula in the language such that the subset or relation consists of exactly those elements satisfying the formula.
- **符号**：$D = \{x \mid \mathcal{M} \models \varphi(x)\}$
- **简明解释**：能用公式唯一刻画的结构子集或关系。
- **典型应用**：模型论、代数、逻辑。
- **相关定理引用**：理论、范畴性。
...
- **英文名**：Theory
- **国际标准表述**：A set of sentences in a formal language, closed under logical consequence.
- **符号**：$T$
- **简明解释**：封闭于逻辑蕴涵的公式集合。
- **典型应用**：模型论、逻辑基础。
- **相关定理引用**：完备性、范畴性。
...
- **英文名**：Categoricity
- **国际标准表述**：A theory is categorical in a cardinality $\kappa$ if all its models of size $\kappa$ are isomorphic.
- **符号**：$\kappa$-categorical
- **简明解释**：某基数下所有模型都同构的理论。
- **典型应用**：结构唯一性、模型分类。
- **相关定理引用**：完备性、同构。

### 完备性（Completeness, Model Theory）

- **英文名**：Completeness (Model Theory)
- **国际标准表述**：A theory is complete if for every sentence in its language, either the sentence or its negation is provable from the theory.
...
## 国际化对齐补全：模型论基础（对齐 Wikipedia/EoM）
...
- 量词消去：每个公式等价于量词自由公式
- 决定性：量词消去⇒可判定性（在有效推演下）
- 示例：实闭域RCF、Presburger算术
...
- Marker, Model Theory；Tent & Ziegler, A Course in Model Theory
- [Wikipedia: Model theory](https://en.wikipedia.org/wiki/Model_theory)
- [EoM: Model theory](https://encyclopediaofmath.org/wiki/Model_theory)
...
- Tarski, A. (1956). Logic, Semantics, Metamathematics. Oxford.（真理论文集）
- Vaught, R. (1962). Denumerable models of complete theories. In Infinitistic Methods, 303–321.（可数模型与谱）
- Chang, C. C., & Keisler, H. J. (1990). Model Theory (3rd ed.). North-Holland.（经典教材）
- Marker, D. (2002). Model Theory: An Introduction. Springer.（入门教材）
- Hodges, W. (1993). Model Theory. Cambridge.（系统教材）

> 相关专题：`./01-紧致性与勒文海姆-斯科伦定理.md`、`./02-非标准模型与无穷小.md`。

---

## 内文引文对照（关键命题）

- 紧致性：若每个有限子集可满足，则整体可满足。 [Gödel 1930; Henkin 1949; Chang–Keisler 1990]
- Löwenheim–Skolem 下行：若在可数语言中有一个无限模型，则有可数模型。 [Skolem 1920; Chang–Keisler 1990]
- Löwenheim–Skolem 上行：存在所有更大基数的扩展模型（适当条件下）。 [Tarski–Vaught 1957; Marker 2002]
- Skolem 悖论：可数模型内部之“不可数”的相对性。 [Skolem 1922; Hodges 1997]
- 完备性与一致性：哥德尔完备性定理（语义可满足⇔语法可证明）；一致性在保守扩展与解释下的传递性。 [Gödel 1930; Henkin 1949; Marker 2002]

> 相关：详见本目录 `01-紧致性与L–S定理.md` 及“Skolem 悖论（简述）”小节。
