...
- [1. 紧致性与勒文海姆-斯科伦定理](#1-紧致性与勒文海姆-斯科伦定理)
  ...
  - [国际化对齐补全：紧致性与Löwenheim–Skolem定理（对齐 Wikipedia/EoM）](#国际化对齐补全紧致性与löwenheimskolem定理对齐-wikipediaeom)
    - [一、紧致性定理](#一紧致性定理)
    - [二、Löwenheim–Skolem定理](#二löwenheimskolem定理)
    - [三、应用与推论](#三应用与推论)
    - [四、重要例子与反例](#四重要例子与反例)
    - [五、参考](#五参考)
...
## 国际化对齐补全：紧致性与Löwenheim–Skolem定理（对齐 Wikipedia/EoM）

### 一、紧致性定理

- 陈述：T的每个有限子集可满足⇒T可满足
- 证明纲要：通过超积构造模型（Łoś定理的应用）
- 等价形式：T不可满足⇒存在有限不可满足子集

### 二、Löwenheim–Skolem定理

- 下LS：无限模型M⇒存在可数初等子模型N≺M
- 上LS：无限模型M⇒对任意基数κ≥|L|+|M|，存在基数κ的初等扩展
- 构造方法：Skolem函数与Skolem闭包
...
- Marker, Model Theory, Ch.2-3；Tent & Ziegler, A Course in Model Theory, Ch.3
