...
实数系 $\mathbb{R}$ 由以下公理系统完全刻画：

#### 1.2.1 域公理 (Field Axioms)

实数集 $\mathbb{R}$ 关于加法 $+$ 和乘法 $\cdot$ 构成一个**域**，即满足：

**加法公理**:
1. **结合律**: $\forall a, b, c \in \mathbb{R}, (a + b) + c = a + (b + c)$
2. **交换律**: $\forall a, b \in \mathbb{R}, a + b = b + a$
3. **零元存在**: $\exists 0 \in \mathbb{R}, \forall a \in \mathbb{R}, a + 0 = a$
4. **负元存在**: $\forall a \in \mathbb{R}, \exists (-a) \in \mathbb{R}, a + (-a) = 0$

**乘法公理**:
5. **结合律**: $\forall a, b, c \in \mathbb{R}, (a \cdot b) \cdot c = a \cdot (b \cdot c)$
6. **交换律**: $\forall a, b \in \mathbb{R}, a \cdot b = b \cdot a$
7. **单位元存在**: $\exists 1 \in \mathbb{R}, 1 \neq 0, \forall a \in \mathbb{R}, a \cdot 1 = a$
8. **逆元存在**: $\forall a \in \mathbb{R}, a \neq 0, \exists a^{-1} \in \mathbb{R}, a \cdot a^{-1} = 1$

**分配律**:
9. **分配律**: $\forall a, b, c \in \mathbb{R}, a \cdot (b + c) = a \cdot b + a \cdot c$

#### 1.2.2 序公理 (Order Axioms)

实数集 $\mathbb{R}$ 上存在一个**全序关系** $\leq$，满足：

1. **自反性**: $\forall a \in \mathbb{R}, a \leq a$
2. **反对称性**: $\forall a, b \in \mathbb{R}, (a \leq b \land b \leq a) \Rightarrow a = b$
3. **传递性**: $\forall a, b, c \in \mathbb{R}, (a \leq b \land b \leq c) \Rightarrow a \leq c$
4. **全序性**: $\forall a, b \in \mathbb{R}, a \leq b \lor b \leq a$

**序与运算的相容性**:
5. **加法保序**: $\forall a, b, c \in \mathbb{R}, a \leq b \Rightarrow a + c \leq b + c$
6. **乘法保序**: $\forall a, b \in \mathbb{R}, (a \leq b \land 0 \leq c) \Rightarrow a \cdot c \leq b \cdot c$

#### 1.2.3 完备性公理 (Completeness Axiom)

**公理 1.2.3 (确界存在定理 / Completeness Axiom)**:
...
1. **域公理**和**序公理**在有理数集 $\mathbb{Q}$ 中也成立。
2. **完备性公理**是实数区别于有理数的本质特征。
3. 有理数集 $\mathbb{Q}$ **不满足**完备性公理（例如，$S = \{x \in \mathbb{Q} : x^2 < 2\}$ 在 $\mathbb{Q}$ 中无上确界）。
4. 此公理是实分析的基石，后续许多重要定理都依赖于它。
5. 实数系的**唯一性**：满足上述所有公理的集合在同构意义下唯一。
