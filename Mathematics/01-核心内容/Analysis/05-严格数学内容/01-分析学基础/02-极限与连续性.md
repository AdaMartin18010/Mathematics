...
theorem intermediate_value
  ...
  -- 证明思路：
  -- 1. 处理两种情况：f a ≤ y ≤ f b 或 f b ≤ y ≤ f a
  -- 2. 使用mathlib4的intermediate_value_Icc定理
  -- 3. 对于f b ≤ y ≤ f a的情况，通过考虑-f来转换
  -- 详细证明参见：Lean/Exercises/Analysis/Real.lean (第397-462行)
...
theorem extreme_value
  ...
  -- 完整证明已在 Lean/Exercises/Analysis/Real.lean 中实现
  -- 证明思路：
  -- 1. 使用紧致性：Set.Icc a b是紧致的
  -- 2. 使用mathlib4的IsCompact.exists_isMaxOn和IsCompact.exists_isMinOn
  -- 详细证明参见：Lean/Exercises/Analysis/Real.lean (第463-486行)
```
