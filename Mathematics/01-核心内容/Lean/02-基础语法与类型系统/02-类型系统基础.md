...
### 智能类型推导 | Intelligent Type Inference
...
-- 简化的类型注解
def simple_add x y := x + y  -- 类型自动推断
```

### 新语法特性 | New Syntax Features
...
- [Lean4语义分析：类型系统语义分析（依赖×宇宙）](../08-Lean4语义分析论证系统/02-Lean4语言语义深度分析/02-类型系统语义分析.md#依赖类型宇宙层级交互--dependent-types--universe-levels)
