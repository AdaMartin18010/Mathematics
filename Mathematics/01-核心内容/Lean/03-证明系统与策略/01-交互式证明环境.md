...
## 可运行脚手架与最佳实践 | Runnable Scaffold & Best Practices

```text
项目初始化步骤
1) 新建目录并创建 `lakefile.lean`、`lean-toolchain`
2) `lake update && lake build`
3) 在 `Main.lean` 中导入 `Mathlib` 并验证编译

最小 `lakefile.lean`
  import Lake
  open Lake DSL
  package «ScaffoldDemo» {}
...
常见问题
- 模块找不到：用 `mathlib4_docs` 搜索最新导入路径
- 版本不兼容：固定 `require mathlib @ <tag>` 与本地 toolchain 一致
- 编译慢：按需导入模块，避免全量导入
```
