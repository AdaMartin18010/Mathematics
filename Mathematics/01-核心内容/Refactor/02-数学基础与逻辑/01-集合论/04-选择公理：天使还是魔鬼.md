...
> 对于 **任意** 一族非空的集合 $\mathcal{F}$，存在一个"选择函数" $f$，其定义域为 $\mathcal{F}$，并且对于 $\mathcal{F}$ 中的每一个集合 $S$，都有 $f(S) \in S$。 [Jech 2003, Chap.5]

**直观解释**:
想象你有一堆装满了东西的抽屉（一族非空集合）。选择公理保证了，你总能从 **每一个** 抽屉里 **同时** 拿出一个东西，组成一个新集合。 [Jech 2003]
...
如果没有AC，现代数学的大厦将会有很多重要支柱崩塌。 [Jech 2003]

AC有许多等价的、在特定领域更便于使用的形式。最重要的两个是：

### 4.2.1. 良序定理 (Well-ordering Theorem)

> 任何集合都可以被 **良序化 (well-ordered)**。 [Zermelo 1908]
...
- **联系**: 这是策梅洛最初提出AC时想要证明的命题。 [Zermelo 1908]
- 如果一个集合可以被良序化，我们就可以通过定义"选择该子集的最小元"来构造出选择函数，从而证明AC。
- 反之亦然。 [Jech 2003, Chap.5]
...
> 设 $(P,\le)$ 是一个非空偏序集，且其中每条链都有上界，则 $P$ 存在极大元。 [Zorn 1935]

### Zorn引理（Zorn's Lemma）国际标准定义

- **英文名**：Zorn's Lemma
- **国际标准表述**：If every chain in a nonempty partially ordered set has an upper bound, then the poset has a maximal element.
- **简明解释**：链上界⇒极大元存在。
- **典型应用**：代数与泛函分析中的极大化构造。
...
- Zermelo, E. (1904/1908). Beweis, dass jede Menge wohlgeordnet werden kann / Untersuchungen über die Grundlagen der Mengenlehre I.（良序定理提出与选择思想；1908: Math. Ann. 65, pp.261–281）
- Zorn, M. (1935). A remark on method in transfinite algebra. Bull. AMS, 41(10), 667–670.（佐恩引理）
- Tarski, A. (1948). Axiom of Choice. In R. L. Goodstein et al.（综述条目）
- Jech, T. (2003). Set Theory (3rd ed.). Springer.（AC与等价：良序定理、Zorn，引文见Chap. 5–6）
- Wagon, S. (1985). The Banach–Tarski Paradox. Cambridge.（悖论专著）

> 返回：`../00-集合论总览.md`；相关：`./03-ZFC公理系统：重建基础.md`。

---

## 内文引文对照（关键命题）

- 良序定理（“Every set can be well-ordered.”）：Zermelo 1908, Math. Ann. 65, pp.261–281；Jech 2003, Chap.5.
- Zorn 引理（极大元等价形式）：Zorn 1935, Bull. AMS 41(10):667–670；Jech 2003, Chap.6.
- AC⇔良序⇔Zorn 等价链：Jech 2003, Chap.5–6（定理编号随版次）；Kunen 2011, Chap. I.
- Banach–Tarski 悖论（非可测分解依赖AC）：Wagon 1985, Ch.1–2.

---

## 典型构造与反例（补充）

- Vitali 集（不可测性）：在 [0,1] 上按“实数相差有理数”分成等价类，借助选择公理在每一类中选一代表，得 Vitali 集 V。可证明 V 不勒贝格可测。 [Vitali 1905; Wagon 1985, Ch.2; Jech 2003]
- 向量空间基（Hamel 基）：一般向量空间存在基的断言与选择公理等价；尤其实数域作为有理数域上的向量空间存在 Hamel 基，但不可构造给出。 [Jech 2003]
- Tychonoff 定理：任意族紧空间的笛卡尔积仍紧（乘积拓扑）。在 ZF 中，Tychonoff 定理与选择公理等价（适当形式）。 [Jech 2003]

> 延伸阅读：Banach–Tarski 分解依赖 AC 的强形式；乌拉姆可测基数与 AC 的张力。 [Jech 2003]
